...
export default class MediaSourcesHandler {
	...
	public get audioDevices() {
		return this._devices.filter(device =>
			device.kind.startsWith("audioinput")
		)
	}
	...
	public async initializeLocalStream(constraints: MediaStreamConstraints = this.defaultConstraints, defaultParamsFromLocalStorage = true) {
		const stream = await this.getMediaDevicesStream(constraints, defaultParamsFromLocalStorage)
		if (stream) {
	...
	public async stopAndRemoveVideo(currentVideoTracks = this.currentVideoTracks) {
		currentVideoTracks.forEach(track => track.enabled = false)
		await new Promise(resolve => setTimeout(resolve, 200));
		currentVideoTracks.forEach(track => { track.stop(); this._currentStream.removeTrack(track) })
	}

	public stopAndRemoveAudio(currentAudioTracks = this.currentAudioTracks) {
		currentAudioTracks.forEach(track => { track.stop(); this._currentStream.removeTrack(track) })
	}
	public stopAudio() {
